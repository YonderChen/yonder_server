...
@Component
public class RefreshSlowChargingOrdersThread extends Thread {
	...
	@Override
	public void run() {
		System.out.println("RefreshSlowChargingOrdersThread---------------begin");
		while (true) {
			List<ChargingOrders> chargingOrdersList = refreshService
					.getChargingOrdersList(ChargingOrders.CHARGE_TYPE_SLOW);
			refreshService.refreshChargingOrderList(chargingOrdersList);
			//计算下一次刷新最短要多长时间，并让线程在这段时间内休眠
			Date minDate = refreshService.getChargingOrdersMinTime(ChargingOrders.CHARGE_TYPE_SLOW);
