...
//		if(session.getLocalAddress().toString().contains(":8000")){
		...
		boolean isComplete = this.isComplete(session, in);
		long end = System.currentTimeMillis();
		System.out.println(ServerHandler.count + ":" + (end - begin));
			return isComplete ? MessageDecoderResult.OK : MessageDecoderResult.NEED_DATA;
//		}else{
...
//		}
	 ...
	 */
	private boolean isComplete(IoSession session, IoBuffer in){
		/*
		 * 先获取HTTP请求的原始报文
		 */
		//判断是否已经收取过一部分报文
		if (session.containsAttribute("dataLength")) {
			int dataLength = NumberUtils.toInt(session.getAttribute("dataLength").toString(), 0);
			if (in.limit() < dataLength) {
				return false;
			} else {
				return true;
			}
		...
		if(message.startsWith("POST")){
			if(message.contains("Content-Length:")){
				...
				if (datas.length >= 2) {
					String head = datas[0];
					String body = datas[1];
					int contentLengthFlagIndex = message.indexOf("Content-Length:") + 15;
					int contentLength = Integer.parseInt(head.substring(contentLengthFlagIndex, head.indexOf("\r\n", contentLengthFlagIndex)).trim());
					//取Content-Length后的字符串
					if(contentLength == 0){
						return true;
					}else if(contentLength > 0){
						//取HTTP_POST请求报文体
						if(contentLength == JadyerUtil.getBytes(body, "UTF-8").length){
							return true;
						} else {
							int headLength = head.getBytes().length;
							int dataLength = headLength + contentLength;
							session.setAttribute("dataLength", dataLength);
						}
					}
				}
			}
